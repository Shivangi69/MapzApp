...
struct AccountAPI {
    ...
    static func getsigninwitoutloader(servciename :String, _ parameters: Parameters?, completion: @escaping (Result<JSON>) -> Void) {
    RequestAPI.call2(servciename, method: .get, parameters: nil) { res in
      switch res {
      case .success:
        if let json = res.value {
          UserDefaults.standard.set(json["access_token"].string, forKey: "access_token")
